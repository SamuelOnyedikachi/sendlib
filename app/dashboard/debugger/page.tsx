...
import { type DebuggerLog, useDebuggerLogs, useInspectEmail } from "@/hooks/useDebugger";
import { redactEmail } from "@/utils/redact";
import { Refresh01Icon, SearchVisualIcon } from "@hugeicons/core-free-icons";
  ...
  return {
    ...
    steps: [
      { key: "received", label: "Request received", ok: true, detail: "Logged by Sendlib." },
      {
        ...
        ok: true,
        skipped: !log.templateSlug,
        detail: log.templateSlug ? `Used ${log.templateSlug}.` : "Custom HTML.",
      },
      {
        key: "variables",
        ...
        ok: true,
        skipped: !log.templateSlug,
        detail: log.templateSlug ? "Template data applied." : "No template variables.",
      },
      {
        key: "gmail",
        label: "Gmail accepted request",
        ok: sent,
        detail: sent ? "Gmail queued the message." : log.error || "Gmail rejected the request.",
      },
      {
        key: "sent",
        label: "Message sent",
        ok: sent,
        detail: sent ? (log.messageId ? `ID ${log.messageId}.` : "Accepted.") : "Not delivered.",
      },
    ],
    issues: sent
      ? []
      : [
          {
            ...
            code: "send_failed",
            title: "Send failed",
            hint: log.error || "Gmail did not accept this message.",
          },
        ],
  };
}
          ...
          <div className="rounded-xl border border-outline-variant bg-surface p-4 space-y-3">
            <div>
              ...
              <p className="text-xs text-secondary mt-0.5">
                Same JSON you POST to /api/send. Nothing is delivered.
              </p>
            </div>
            <Textarea
              value={payload}
          ...
          <div className="rounded-xl border border-outline-variant bg-surface p-4">
            ...
            {inspect.data ? (
              ...
              <p className="text-xs text-secondary leading-relaxed">
                We check missing fields, broken <code>{"{{variables}}"}</code>, invalid HTML, bad
                addresses, oversized markup, images without alt, and weak links.
              </p>
            )}
          </div>
        </div>
            ...
            {isLoading ? (
              ...
              <div className="px-5 py-10 text-center">
                <HugeiconsIcon
                  ...
                  color="currentColor"
                  strokeWidth={1.5}
                  className="mx-auto text-secondary/40 mb-2"
                />
                    ...
                    <li key={log.id}>
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-semibold text-primary-sendlib truncate">
                            {log.subject}
                          </span>
                          <HealthBadge
                            health={
                              log.debug?.health ?? (log.status === "failed" ? "failed" : undefined)
                            }
                          />
                        </div>
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h2 className="text-base font-headline-md font-bold text-primary-sendlib truncate">
                      {selected?.subject}
                    </h2>
                    <p className="text-xs text-secondary mt-0.5">
  ...
  return (
    <button
      ...
      className={`h-8 px-3 rounded-md text-xs font-semibold transition-colors ${
        active
          ? "bg-surface-container-low text-primary-sendlib"
