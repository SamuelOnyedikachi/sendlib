...
import { EmailLog, useEmailLogs } from "@/hooks/useEmailLogs";
import { useConnectGmail, useGmailAccounts } from "@/hooks/useGmailAccounts";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        >
          ...
          <p className="text-secondary font-body-md mt-1">
            Your API relay service is active and running.
          </p>
        </motion.div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        {[
          {
            label: "Emails sent",
            value: isLoadingLogs ? null : totalEmailsSent.toLocaleString(),
            hint: "All time",
          },
          {
            label: "Gmail accounts",
            value: isLoadingAccounts ? null : String(connectedGmailsCount),
            hint:
          ...
          {
            label: "Last 7 days",
            value: isLoadingAnalytics ? null : weekSent.toLocaleString(),
            hint: `${weekFailed} failed`,
          },
          {
            label: "Delivery",
            value: isLoadingAnalytics ? null : `${successRate}%`,
            hint: weekTotal === 0 ? "No traffic yet" : "Sent vs failed",
          },
        ].map((stat) => (
          <div
            key={stat.label}
            className="rounded-xl border border-outline-variant bg-surface px-4 py-3.5"
          >
            ...
            {stat.value === null ? (
              ...
              <p className="text-2xl font-headline-md font-bold tracking-tight text-on-background mt-1 tabular-nums">
                {stat.value}
              </p>
            )}
        ...
        >
          <Card className="h-full border border-outline-variant bg-surface shadow-none rounded-xl">
            <CardHeader className="flex flex-row items-start justify-between gap-3 pb-2 px-5 pt-4">
              <div>
                <CardTitle className="text-sm font-headline-md font-bold text-primary-sendlib">
                  Send Volume
                </CardTitle>
                  ...
                  <div className="flex-1 min-w-0">
                          ...
                          return (
                            ...
                            >
                              <div className="w-full max-w-[22px] h-full rounded-sm bg-outline-variant/10 overflow-hidden flex flex-col justify-end">
                                {failH > 0 && (
                                  <div className="w-full bg-sky-400" style={{ height: failH }} />
                                )}
                                {sentH > 0 && (
                                  <div
                    ...
                    <div className="flex gap-1.5 sm:gap-2.5 px-1 mt-1.5">
                      {volume.map((day) => (
                        <div
                          key={day.date}
                          className="flex-1 text-center text-[10px] font-mono text-secondary truncate"
                        >
                          {day.label}
                        </div>
                      ))}
          ...
          <Card className="h-full border border-outline-variant bg-surface shadow-none rounded-xl overflow-hidden">
            <CardHeader className="pb-2 px-5 pt-4">
              ...
              <p className="text-[11px] text-secondary mt-0.5">
                Per connected Gmail, resets UTC midnight
              </p>
            </CardHeader>
            <CardContent className="px-5 pb-4 pt-1">
              {isLoadingAnalytics ? (
                    ...
                    return (
                      <div key={account.email} className="space-y-1.5">
                        <div className="flex items-center justify-between gap-2">
                          <span
                            className="text-xs font-semibold text-primary-sendlib truncate"
                            title={account.email}
                          >
        ...
        >
          <Card className="h-full flex flex-col border border-outline-variant bg-surface shadow-none rounded-xl overflow-hidden hover:bg-surface-container-low transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-4">
              <CardTitle className="text-lg font-headline-md font-bold text-primary-sendlib">
                Recent Email Logs
              </CardTitle>
              <div className="flex items-center gap-1">
                <Link
                  href="/dashboard/debugger"
                  className="text-xs font-semibold text-secondary hover:text-primary-sendlib px-2"
                >
                  Debugger
                </Link>
                <Button
                ...
                >
                  <HugeiconsIcon
                    ...
                    strokeWidth={1.5}
                    className={
                      isFetchingLogs ? "animate-spin text-primary-sendlib" : "text-secondary"
                    }
                  />
                </Button>
              </div>
              ...
              <Table>
                <TableHeader className="bg-surface border-y border-outline-variant/30">
                  <TableRow className="border-outline-variant/30 hover:bg-transparent">
                    <TableHead className="font-label-sm font-bold pl-6 text-primary-sendlib">
                      From
                    </TableHead>
                    <TableHead className="font-label-sm font-bold text-primary-sendlib">
                      To
                    </TableHead>
                    <TableHead className="font-label-sm font-bold text-primary-sendlib">
                      Subject
                    ...
                    <TableHead className="font-label-sm font-bold text-right pr-6 text-primary-sendlib">
                      Date
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {isLoadingLogs || isFetchingLogs ? (
                    Array.from({ length: 3 }).map((_, i) => (
                      <TableRow key={i} className="border-outline-variant/30">
                        ...
                        <TableCell>
                          <Skeleton className="h-5 w-16 bg-outline-variant/20" />
                        </TableCell>
                        <TableCell className="pr-6">
                          <Skeleton className="h-4 w-20 ml-auto bg-outline-variant/20" />
                        </TableCell>
                      </TableRow>
                    ))
                  ) : emailLogs.length === 0 ? (
                      ...
                      <TableCell colSpan={5} className="text-center py-10">
                        <div className="flex flex-col items-center justify-center max-w-[380px] mx-auto space-y-2.5">
                          <HugeiconsIcon
                            ...
                            color="currentColor"
                            strokeWidth={1.5}
                            className="text-primary-sendlib opacity-40 mb-0.5"
                          />
                          <h4 className="text-base font-headline-md font-bold text-primary-sendlib">
                            No email logs found
                          </h4>
                          <p className="text-xs text-secondary leading-relaxed">
                            Sent emails will appear here automatically once you send through the
                            Sendlib API.
                          </p>
                          <div className="pt-1 flex items-center gap-2.5">
                            <Link href="/docs/send">
                              <Button
                                variant="outline"
                                size="sm"
                                className="rounded-lg font-label-sm border border-outline-variant bg-surface text-primary-sendlib shadow-xs hover:bg-surface-container-low px-3.5 h-8"
                              >
                                View API Docs
                              </Button>
                            </Link>
                            <Link href="/dashboard/templates">
                              <Button
                                size="sm"
                                className="rounded-lg font-label-sm bg-surface-container-low border border-outline-variant/60 hover:bg-surface-container text-white shadow-sm px-3.5 h-8"
                              >
                                Templates
                              </Button>
                            </Link>
                    ...
                    emailLogs.map((log: EmailLog, index: number) => (
                      <TableRow
                        key={log.id || index}
                        className="border-outline-variant/30 transition-colors hover:bg-surface-container-low group"
                      >
                        ...
                        <TableCell
                          className="py-4 text-xs text-secondary truncate max-w-[150px]"
                          title={log.subject}
                        >
                          {log.subject}
                        </TableCell>
          ...
          >
            <Card className="h-full flex flex-col border border-outline-variant bg-surface shadow-none rounded-xl hover:bg-surface-container-low transition-colors">
              <CardHeader className="pb-3">
                <CardTitle className="text-xs font-label-xs uppercase tracking-wider text-secondary">
                  Setup Guide
                </CardTitle>
              </CardHeader>
              <CardContent className="flex-1 flex flex-col justify-center pb-6 px-6 space-y-0 gap-1">
                ...
                <div className="flex gap-4 items-start">
                  <div className="flex flex-col items-center shrink-0">
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-colors duration-300 z-10 ${
                        connectedGmailsCount > 0
                          ? "bg-primary-sendlib text-surface-container-lowest"
                          : "border border-outline-variant bg-surface text-primary-sendlib"
                      }`}
                    >
                      1
                    </div>
                    <div
                      className={`w-0.5 h-10 my-1 transition-colors duration-300 ${connectedGmailsCount > 0 ? "bg-primary-sendlib" : "bg-outline-variant/30"}`}
                    />
                  </div>
                  <div className="pt-0.5">
                    <h4
                      className={`font-bold text-sm transition-colors duration-300 ${connectedGmailsCount > 0 ? "text-primary-sendlib" : "text-secondary"}`}
                    >
                      Connect Gmail
                    </h4>
                    <p className="text-xs text-secondary mt-0.5 leading-snug">
                      Authorize your Google account for sending.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-4 items-start">
                  <div className="flex flex-col items-center shrink-0">
                    ...
                    <div
                      className={`w-0.5 h-10 my-1 transition-colors duration-300 ${activeKeysCount > 0 ? "bg-primary-sendlib" : "bg-outline-variant/30"}`}
                    />
                  </div>
                  <div className="pt-0.5">
                    <h4
                      className={`font-bold text-sm transition-colors duration-300 ${activeKeysCount > 0 ? "text-primary-sendlib" : "text-secondary"}`}
                    >
                      Get API Key
                    </h4>
                    <p className="text-xs text-secondary mt-0.5 leading-snug">
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="pt-0.5">
                    <h4
                      className={`font-bold text-sm transition-colors duration-300 ${totalEmailsSent > 0 ? "text-primary-sendlib" : "text-secondary"}`}
                    >
                      Send transactional emails
                    </h4>
                    <p className="text-xs text-secondary mt-0.5 leading-snug">
                      Use our REST API to send transactional emails seamlessly.
                    </p>
                  </div>
                </div>
              </CardContent>
      ...
      <Sheet open={!!newKeyDialog} onOpenChange={() => setNewKeyDialog(null)}>
        <SheetContent
          side="right"
          className="w-full sm:max-w-md border-l border-outline-variant p-6 bg-surface-container-lowest"
        >
          <SheetHeader className="p-0 mb-6">
            ...
            <SheetDescription className="text-secondary text-sm">
              Copy your new secret key below. For security reasons, you won&apos;t be able to view
              it again.
            </SheetDescription>
          </SheetHeader>
      ...
      <Dialog open={milestoneDialogOpen} onOpenChange={setMilestoneDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader className="mb-2">
            ...
            <DialogDescription className="text-secondary text-sm leading-relaxed mt-1">
              You&apos;ve successfully connected your Gmail account! Next up: Generate an API Key to
              authorize email requests from your app.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-row gap-3 mt-4 pt-4 border-t border-outline-variant/30">
            ...
            <Link href="/dashboard/keys" className="flex-1">
              <Button className="w-full rounded-lg font-label-sm bg-surface-container-low border border-outline-variant/60 hover:bg-surface-container text-white cursor-pointer">
                Go to API Keys
              </Button>
            </Link>
          </div>
