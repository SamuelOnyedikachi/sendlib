  ...
  try {
    ...
    if (count >= maxAccounts) {
      return NextResponse.json(
        { success: false, message: `You have reached the maximum of ${maxAccounts} connected Gmail accounts.${effectivePlan === "free" ? " Upgrade to Pro to connect up to 50 accounts." : ""}` },
        { status: 429 }
      );
    }
